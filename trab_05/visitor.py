...
class ErrorMessages:
    ...
    def var_not_defined_error(var_id: str) -> str:
        return f"Error: variable '{var_id}' is not defined."
    ...
    def var_type_error(var_id: str, expt_type: str | None, rcv_type: str | None) -> str:
        return f"Error: variable '{var_id}' expected type '{expt_type}' but got '{rcv_type}'."

    @staticmethod
    def fun_call_arg_type_error(
            fun_id: str, arg_type: str | None, param_id: str, recv_type: str | None
    ) -> str:
        return f"Error: function '{fun_id}' expected '{arg_type}' for param '{param_id}' but got '{recv_type}'."

    @staticmethod
    def fun_call_not_defined_error(fun_id: str) -> str:
        return f"Error: function '{fun_id}' is not defined."

    @staticmethod
    def fun_arg_count_error(fun_id: str, expt_count: int, rcv_count: int) -> str:
        return f"Error: function '{fun_id}' expected '{expt_count}' arguments but got '{rcv_count}'."

    @staticmethod
    def fun_ret_type_error(fun_id: str, fun_type: str, rcv_type: str) -> str:
        return f"Error: function '{fun_id}' return type is '{fun_type}' but is returning type '{rcv_type}'."

    @staticmethod
    def fun_not_ret_error(fun_id: str, fun_type: str) -> str:
        return f"Error: function '{fun_id}' has return type '{fun_type}' but is missing a 'return' statement."

    @staticmethod
    def var_redeclaration_error(var_id: str) -> str:
        return f"Error: variable '{var_id}' is already declared."

    @staticmethod
    def expression_type_mismatch_error(fun_id: str) -> str:
        return f"Error: in function '{fun_id}' found type mismatch in expression."
...
class Visitor(MiniCVisitor):
    ...
    def visitFunction_call(self, ctx: MiniCParser.Function_callContext):
        ...
        if ctx.argument_list():
            ...
            if provided_count != expected_count:
                self.__add_error(ErrorMessages.fun_arg_count_error(_id, expected_count, provided_count), ctx)
                return None
            ...
            for i in range(provided_count):
                ...
                if binary.binary(0) and binary.binary(1):
                    ...
                    if arg_sym.type != expr_type:
                        self.__add_error(
                            ErrorMessages.fun_call_arg_type_error(_id, arg_sym.type, arg_sym.id, expr_type), ctx)
                    continue

                primary = binary.unary().primary()
                arg = primary.getText()
                ...
                if primary.IDENTIFIER():
                    ...
                    if arg_sym.type != provided_type:
                        self.__add_error(
                            ErrorMessages.fun_call_arg_type_error(_id, arg_sym.type, arg_sym.id, provided_type), ctx)

                elif primary.CONSTANT_INT():
                    if arg_sym.type != "int":
                        self.__add_error(ErrorMessages.fun_call_arg_type_error(_id, arg_sym.type, arg_sym.id, "int"),
                                         ctx)

                elif primary.CONSTANT_CHAR():
                    if arg_sym.type != "char":
                        self.__add_error(ErrorMessages.fun_call_arg_type_error(_id, arg_sym.type, arg_sym.id, "char"),
                                         ctx)
                    ...
                    if arg_fun_id not in self.fun_sym_table:
                        self.__add_error(ErrorMessages.fun_call_not_defined_error(arg_fun_id), ctx)
                        continue
                    ...
                    if arg_sym.type != arg_fun_type:
                        self.__add_error(
                            ErrorMessages.fun_call_arg_type_error(_id, arg_sym.type, arg_sym.id, arg_fun_type), ctx)
    ...
    def visitAssignment_statement(self, ctx: MiniCParser.Assignment_statementContext):
        ...
        if expr_type is None or expr_type != expected_type:
            self.__add_error(ErrorMessages.var_type_error(_id, expected_type, expr_type), ctx)
    ...
    def visitBinary(self, ctx: MiniCParser.BinaryContext):
        if ctx.binary(0) and ctx.binary(1):
            ...
            if l_type != r_type:
                self.__add_error(ErrorMessages.expression_type_mismatch_error(self.current_scope), ctx)
                return None
